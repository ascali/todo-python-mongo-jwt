...
secret_key = 'qwertyasd123'

app.config['MONGO_URI'] = 'mongodb://localhost:8081/todo'
...
def register():
    ...
    if mongo.db['user'].find_one({'username': data['username']}):
        ...
        user_id = mongo.db['user'].insert_one({'username': data['username'], 'password': generate_password_hash(data['password'])})
...
def add_todo_item():
    try:
        ...
        todo_id = mongo.db['todos'].insert_one({'title': data['title'], 'completed': '0', 'user_id': user_id})
...
def update_todo_item():
     try:
         ...
         data_token = jwt.decode(data['token'], secret_key, algorithms=['HS256'], verify=True)
         user_id = data_token['_id']
         todo = mongo.db['todos'].find_one({'_id': ObjectId(data['todo_id']), 'user_id': user_id})
         if not todo:
